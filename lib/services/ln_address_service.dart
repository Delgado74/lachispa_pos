...
class LNAddressService {
  ...
  void setAuthHeaders(String invoiceKey, String adminKey) {
    ...
    _debugLog('[LN_ADDRESS_SERVICE] - Invoice Key: ${invoiceKey.substring(0, 8)}...');
    _debugLog('[LN_ADDRESS_SERVICE] - Admin Key: ${adminKey.substring(0, 8)}...');
  }
  ...
  void setWalletAuthHeaders(String adminKey) {
    ...
    _debugLog('[LN_ADDRESS_SERVICE] Setting up wallet admin key authentication...');
    
    _dio.options.headers['X-Api-Key'] = adminKey;
    _dio.options.headers['Authorization'] = 'Bearer $adminKey';
    
    _debugLog('[LN_ADDRESS_SERVICE] Admin key configured: ${adminKey.substring(0, 8)}...');
  }
    ...
    for (final endpoint in endpoints) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          if (data is List) {
            final addresses = data.map((json) => LNAddress.fromJson(json, _baseUrl)).toList();
            _debugLog('[LN_ADDRESS_SERVICE] ✅ ${addresses.length} Lightning Addresses obtained from $endpoint');
            return addresses;
          } else {
            _debugLog('[LN_ADDRESS_SERVICE] Unexpected response structure at $endpoint: $data');
        ...
        if (e is DioException && e.response?.statusCode != null) {
          _debugLog('[LN_ADDRESS_SERVICE] Status: ${e.response?.statusCode}, Data: ${e.response?.data}');
        }
        continue; // Try next endpoint
      }
    }
  ...
  Future<List<LNAddress>> getLNAddressesForWallet(String walletId) async {
    try {
      ...
      final walletAddresses = allAddresses.where((address) => address.walletId == walletId).toList();
      
      _debugLog('[LN_ADDRESS_SERVICE] ${walletAddresses.length} Lightning Addresses for wallet $walletId');
      ...
      try {
        ...
        if (e is DioException) {
          ...
          if (e.type == DioExceptionType.connectionError || 
            ...
            _debugLog('[LN_ADDRESS_SERVICE] Real connectivity error: ${e.type}');
            return false;
          }
        }
    ...
    try {
      ...
      for (final endpoint in endpoints) {
        try {
          ...
          if (response.statusCode == 200) {
            _debugLog('[LN_ADDRESS_SERVICE] ✅ LNURLP permissions verified at: $endpoint');
            return true;
          }
        } catch (e) {
          _debugLog('[LN_ADDRESS_SERVICE] Error en endpoint $endpoint: $e');
          if (e is DioException) {
            _debugLog('[LN_ADDRESS_SERVICE] Status Code: ${e.response?.statusCode}');
            _debugLog('[LN_ADDRESS_SERVICE] Response Data: ${e.response?.data}');
            // If we get 404, extension is not installed
            if (e.response?.statusCode == 404) {
              _debugLog('[LN_ADDRESS_SERVICE] ❌ LNURLP extension not found (404)');
              return false;
            }
            // For 401/403, extension exists but has auth problem
            if (e.response?.statusCode == 401 || e.response?.statusCode == 403) {
              _debugLog('[LN_ADDRESS_SERVICE] ⚠️ Authentication problem (${e.response?.statusCode}), but extension available');
  ...
  List<String> _getOptimizedEndpoints() {
    ...
    if (serverUrl.contains('lachispa.me')) {
      ...
      _debugLog('[LN_ADDRESS_SERVICE] ✅ Using official endpoint: /lnurlp/api/v1/links');
      return [
        '/lnurlp/api/v1/links',           // ✅ WORKS on LaChispa
      ];
    }
    
    if (serverUrl.contains('btclake.org') || serverUrl.contains('btclake.com')) {
      _debugLog('[LN_ADDRESS_SERVICE] 🎯 Detected server: BTCLake');
      _debugLog('[LN_ADDRESS_SERVICE] 🧪 TESTING actual BTCLake capabilities...');
      return [
    ...
    _debugLog('[LN_ADDRESS_SERVICE] 🔍 Standard LNBits server - testing standard endpoint');
    return [
  ...
  Future<LNAddress> createLNAddress({
    ...
    try {
      ...
      _debugLog('[LN_ADDRESS_SERVICE] 📋 Endpoints to try: ${endpoints.length}');
      for (int i = 0; i < endpoints.length; i++) {
        final endpoint = endpoints[i];
        try {
          _debugLog('[LN_ADDRESS_SERVICE] 🔄 [${i+1}/${endpoints.length}] Trying: $endpoint');
          ...
          _debugLog('[LN_ADDRESS_SERVICE] ✅ Successful response: ${response.statusCode}');
          _debugLog('[LN_ADDRESS_SERVICE] 📥 Response data: ${response.data}');

          if (response.statusCode == 201 || response.statusCode == 200) {
            final lnAddress = LNAddress.fromJson(response.data, _baseUrl);
            _debugLog('[LN_ADDRESS_SERVICE] ⚡ Lightning Address created successfully: ${lnAddress.fullAddress}');
            _debugLog('[LN_ADDRESS_SERVICE] 🎯 Successful endpoint: $endpoint (works on this server)');
            return lnAddress;
          }
        } on DioException catch (e) {
          final isLast = (i == endpoints.length - 1);
          _debugLog('[LN_ADDRESS_SERVICE] ❌ [${i+1}/${endpoints.length}] Failed endpoint $endpoint');
          _debugLog('[LN_ADDRESS_SERVICE] 📊 Error type: ${e.type}');
          _debugLog('[LN_ADDRESS_SERVICE] 🔢 Status Code: ${e.response?.statusCode}');
          _debugLog('[LN_ADDRESS_SERVICE] 📄 Error Response: ${e.response?.data}');
          _debugLog('[LN_ADDRESS_SERVICE] 🔗 Error Message: ${e.message}');
          
          if (e.response?.statusCode == 404) {
            _debugLog('[LN_ADDRESS_SERVICE] 🚫 404: Endpoint does not exist on this server');
          } else if (e.response?.statusCode == 401) {
            _debugLog('[LN_ADDRESS_SERVICE] 🔐 401: Authentication error');
          } else if (e.response?.statusCode == 403) {
            _debugLog('[LN_ADDRESS_SERVICE] 🚷 403: No permissions for this endpoint');
          } else if (e.type == DioExceptionType.connectionError) {
            _debugLog('[LN_ADDRESS_SERVICE] 🌐 Connection error (CORS detected)');
            _debugLog('[LN_ADDRESS_SERVICE] ⚠️ This is the correct endpoint but browser blocks the request');
            _debugLog('[LN_ADDRESS_SERVICE] 💡 Solution: Contact administrator to enable CORS');
          }
          
          if (isLast) {
            _debugLog('[LN_ADDRESS_SERVICE] ❌ Last endpoint failed - no more options');
            rethrow;
          } else {
            _debugLog('[LN_ADDRESS_SERVICE] ⏭️ Continuing with next endpoint...');
            continue;
          }
        } catch (e) {
      ...
      if (e is DioException) {
        ...
        if (e.type == DioExceptionType.connectionError) {
          if (kIsWeb) {
            ...
            throw Exception('Connection error. Check your internet and server availability.');
          }
        }
        ...
        if (e.type == DioExceptionType.receiveTimeout) {
          throw Exception('Timeout receiving data. Server did not respond in time.');
        }
  ...
  Future<LNAddress> getLNAddress(String id) async {
    try {
      ...
      if (response.statusCode == 200) {
        final lnAddress = LNAddress.fromJson(response.data, _baseUrl);
        _debugLog('[LN_ADDRESS_SERVICE] Lightning Address obtained: ${lnAddress.fullAddress}');
        return lnAddress;
      } else {
  ...
  Future<LNAddress> updateLNAddress(String id, {
    String? username,
    String? description,
    bool? isActive,
  }) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _debugLog('[LN_ADDRESS_SERVICE] Lightning Address updated: ${updated.fullAddress}');
        return updated;
      } else {
  ...
  Future<bool> deleteLNAddress(String id) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 204) {
        _debugLog('[LN_ADDRESS_SERVICE] Lightning Address deleted successfully');
        return true;
      } else {
  ...
  Future<bool> isUsernameAvailable(String username, String walletId) async {
    try {
      ...
      return !addresses.any((address) => address.username.toLowerCase() == username.toLowerCase());
    } catch (e) {
  ...
  Future<bool> checkExtensionAvailability() async {
    try {
      _debugLog('[LN_ADDRESS_SERVICE] Checking LNURLP extension availability...');
      ...
      for (final endpoint in endpoints) {
        try {
          ...
          final response = await _dio.get(endpoint);
          if (response.statusCode == 200) {
            _debugLog('[LN_ADDRESS_SERVICE] ✅ LNURLP extension available at: $endpoint');
            return true;
          }
        } catch (e) {
          _debugLog('[LN_ADDRESS_SERVICE] Endpoint $endpoint not available: $e');
          continue;
        }
      }
      ...
      if (e.toString().contains('401') || e.toString().contains('403')) {
        _debugLog('[LN_ADDRESS_SERVICE] Extension exists but authentication error');
  ...
  Future<String> resolveLightningAddress(String lightningAddress) async {
    try {
      _debugLog('[LN_ADDRESS_SERVICE] Resolving Lightning Address: $lightningAddress');
      ...
      if (_baseUrl.contains(domain)) {
        try {
          _debugLog('[LN_ADDRESS_SERVICE] Using well-known endpoint for: $username');
          final response = await _dio.get('/lnurlp/api/v1/well-known/$username');
          
          if (response.statusCode == 200 && response.data != null) {
            final lnurl = response.data['lnurl'] ?? response.data['callback'];
            if (lnurl != null) {
              _debugLog('[LN_ADDRESS_SERVICE] LNURL obtained from well-known: $lnurl');
              return lnurl;
            }
          }
        } catch (e) {
  ...
  String _encodeLNURL(String url) {
    try {
      ...
      _debugLog('[LN_ADDRESS_SERVICE] Bits convertidos: ${convertedBits.length} elementos');
      
      // Encode in bech32 with prefix 'lnurl'
      final lnurl = _bech32Encode('lnurl', convertedBits);
      
      _debugLog('[LN_ADDRESS_SERVICE] LNURL final: ${lnurl.substring(0, 20)}...${lnurl.substring(lnurl.length - 10)}');
      ...
      for (final endpoint in endpoints) {
        try {
          ...
          if (e.type == DioExceptionType.connectionError && kIsWeb) {
            ...
            result['technical_details'] = 'XMLHttpRequest CORS error at endpoint: $endpoint';
            return result;
          }
          ...
          if (e.response?.statusCode == 401 || e.response?.statusCode == 403) {
            ...
            result['technical_details'] = 'Authentication error ${e.response?.statusCode}';
            return result;
          }
        }
      }
  ...
  Future<bool> verifyWalletOwnership(String walletId) async {
    _debugLog('[LN_ADDRESS_SERVICE] 🔧 FORCING verifyWalletOwnership = true (CACHE BYPASS)');
